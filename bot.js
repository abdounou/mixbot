...
client.on("guildMemberAdd", member => {
let welcomer = member.guild.channels.find('name', `silver`)
...
client.on("message", function(message){
	if (message.content.startsWith(prefix + "level")) {
		...
		.addField("الليفل :", [message.author.id].Level)
		.addField("الأكس بي :",Math.floor([message.author.id].Xp))
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("493103430768787482");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("548464174230011904");
    if (!channel) {
