...
『!skin /لإضهار سكنك في ماين كرافت』
『!top / لإضهار قائمة التوب فويس والتوب الكتابي』
**
...
『!unpause / لموآصلة الإغنية بعد إيقآفهآ مؤقتا』
...
client.on('message', async message => {
    ...
    const { body: ava } = await get(auth.displayAvatarURL.replace(imageUrlRegex, "?size=128"));
    ...
    let inviter =client.users.get(inv[mem.id+message.guild.id].inviter);
        ...
        const { body: bot } = await get(message.guild.iconURL.replace(imageUrlRegex, "?size=128"));


    let canvas = new Canvas(417, 181)
    .addImage(img, 0, 0, 417, 181)
    .addRoundImage(bot, 7, 1, 29, 29, 25)
    .setShadowColor("rgba(22, 22, 22, 1)") // This is a nice colour for a shadow.
    .setShadowOffsetY(3) // Drop the shadow by 5 pixels.
  .setShadowBlur(03) // Blur the shadow by 10.
  .save()
    .addRoundImage(ava, 320, 55, 78, 78, 39)
    .setTextAlign("center")
    .setTextFont("8pt Cairo")
    .setColor((mem.highestRole.hexColor === "#000000") ? "#ffffff" : mem.highestRole.hexColor)
    .addText(name, 360, 162)
    .setColor("#FFFFFF")
    .addText(createdAt.toFixed(), 192, 77)
    .addText((joinedAt.toFixed().length >= 3) ? joinedAt.toFixed() : joinedAt.toFixed() + "D", 257.5, 77)
    .addText("voc", 195, 130)
    .addText("tex", 258, 130)
    .addText(`${inv[mem.id+message.guild.id].totalSecs} Sec`, 205, 163)
    .addText((invites === 1 || invites === 0 ) ? invites + " Member" : invites + " Members", 120, 128)
    .addText((inviter) ? inviter.username : "Not stored", 110, 77)
    if(inviter) {
            const { body: buffer } = await get(inviter.avatarURL.replace(imageUrlRegex, "?size=128"))
...
client.login('NTI5NjA5NTM1NTQ4MTYyMDU5.DyOrwA.fvbdO_o3Xl2a-IDvqA0Joe');
